...
The eigenstates of $H$ are described by a set of non-negative integers defined for each decoupled mode. 
...
When the interaction between fields is spatially locally like this
...
If we regard that $\varphi(x)$ is a coordinate variable for each $x$, the conjugate momentum to $\varphi$ is
...
Let us again assume that $\omega(k)$ are non-negative for all $k$, so that we replace this by $\omega^2(k)$. 

Rewriting the Hamiltonian in terms of these fields and functions, we obtain

\begin{align}
H = \frac{1}{2}\int\frac{d^2 k}{(2\pi)^2}\left[\tilde{\Pi}(-k)\tilde{\Pi}(k)+\omega^2(k)\tilde{\varphi}(-k)\tilde{\varphi}(k)\right] = \frac{1}{2}\int\frac{d^2 k}{(2\pi)^2}\left[\tilde{\Pi}^\dagger (k)\tilde{\Pi}(k)+\omega^2(k)\tilde{\varphi}^\dagger(k)\tilde{\varphi}(k)\right]
\end{align}
...
into $\{P(1),P(2),\cdots P(n)\}$. We take the sum over all possible permutations here. 
...
Here $P$ is again a permutation among $n$-particles, but, at this time, we have $(-1)^P$ ($(-1)$ to the power of $P$). This factor takes $+1$, if the permutation $P$ is given by a product of even-number of transpositions, where a transposition is an exchange between a pair of two particle. 
...
Because of this, asymmetrized property, $n$-particles in the wavefunction are essentially indistinguishable from one another. 
...
\begin{align}
\frac{1}{n!}\sum_{\alpha_1}\cdots\sum_{\alpha_n}|\alpha_1,\cdots,\alpha_n\rangle\langle\alpha_1,\cdots,\alpha_n| = 1
\end{align}
...
A destruction operator converts an $n$-particle states into an $(n-1)$-particle states. 
...
\[a^\dagger(\varphi_1)a^\dagger(\varphi_2) = \xi a^\dagger(\varphi_2)a^\dagger(\varphi_1) \]

Namely, the following relations holds true for arbitrary $\psi_1,\cdots,\psi_n$

\[a^\dagger(\varphi_1)a^\dagger(\varphi_2)|\psi_1,\cdots,\psi_n\rangle = \xi a^\dagger(\varphi_2)a^\dagger(\varphi_1) |\psi_1,\cdots,\psi_n\rangle\]
